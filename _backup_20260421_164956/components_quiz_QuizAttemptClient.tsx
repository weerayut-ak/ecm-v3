...
const PENALTY_SECONDS = 60 // หักครั้งละ 60 วินาที
...
export default function QuizAttemptClient({ quiz, questions, userId }: { quiz: Quiz; questions: Question[]; userId: string }) {
  ...
  useLeaveGuard({
    ...
    onLeave: () => {
      setWarnings(w => {
        const next = w + 1
        toast.error(`⚠️ ออกจากหน้าแบบทดสอบ! (ครั้งที่ ${next}) ถูกหักเวลา ${PENALTY_SECONDS} วินาที`, { duration: 5000 })
        // หักเวลา
        if (quiz.time_limit) {
          timer.deduct(PENALTY_SECONDS)
        }
        // ถ้าออก 3 ครั้งขึ้นไป ล็อค
        if (next >= 3) {
          setBlocked(true)
          toast.error('🚫 ถูกล็อคเนื่องจากออกจากหน้าแบบทดสอบเกิน 3 ครั้ง', { duration: 0 })
        }
        return next
      })
    },
  ...
  async function doSubmit() {
    ...
    // อัพเดท session เป็น submitted
    await supabase.from('quiz_sessions').update({ status: 'submitted' })
  ...
  function confirmLeave() {
    if (confirm('⚠️ ออกจากแบบทดสอบ? คำตอบจะไม่ถูกบันทึก')) {
      timer.stop()
      supabase.from('quiz_sessions').update({ status: 'left' })
        .eq('student_id', userId).eq('quiz_id', quiz.id)
      ...
      <div style={{ maxWidth: 480, margin: '80px auto', textAlign: 'center' }}>
        <div className="card" style={{ padding: 40, border: '2px solid var(--red)' }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-2)', lineHeight: 1.7, marginBottom: 20 }}>
            ออกจากหน้าแบบทดสอบเกิน 3 ครั้ง<br />กรุณาติดต่อครูผู้สอน
          </p>
          <button className="btn btn-danger" onClick={() => router.push('/dashboard/quizzes')}>
            กลับหน้าหลัก
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 20, background: 'var(--bg)', paddingBottom: 12, marginBottom: 8 }}>
        ...
        {warnings > 0 && (
          <div className="alert alert-danger" style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span>คำเตือน: ออกจากหน้าแบบทดสอบแล้ว {warnings} ครั้ง {warnings >= 2 && '— อีก 1 ครั้งจะถูกล็อค!'}</span>
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 24, paddingBottom: 16 }}>
        <button className="btn" style={{ flex: 1, justifyContent: 'center', color: 'var(--red)' }} onClick={confirmLeave}>ออกจากแบบทดสอบ</button>
